...
class Main{
    ...
    public static void main(String[] args) {
        ...
        // recruiting DaVinci
        DaVinci drawer = new DaVinci(models);
...
class DaVinci{
    private ExampleModel[] modelHandler;
    public DaVinci(ExampleModel[] modelHandler){
        this.modelHandler = modelHandler;
    }

    public void DrawModel(GridWindow vis, int iModel) {
        for (int x = 0; x < this.modelHandler[iModel].xDim; x++) {
            for (int y = 0; y < this.modelHandler[iModel].yDim; y++) {
                ExampleCell drawMe = this.modelHandler[iModel].GetAgent(x, y);
                if (drawMe != null) {
                    vis.SetPix(x + iModel * this.modelHandler[iModel].xDim, y, drawMe.type);
                } else {
                    vis.SetPix(x + iModel * this.modelHandler[iModel].xDim, y, HeatMapRGB(this.modelHandler[iModel].drug.Get(x, y)));
                }
            }
        }
    }
