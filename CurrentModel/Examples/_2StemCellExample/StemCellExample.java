...
class CACell extends AgentSQ2Dunstackable<StemCellCA> {
    ...
    void Divide(int iChildLoc) {
        ...
        // division of nonStem cells
        // if stem? --> then follows stem rules to divide
        // if nonStem? --> then follows nonStem rules to divide

        boolean stemChild;
        int divsChild;
        if(stem){
            if (G.rn.Double()< G.STEM_DIV_PROB){
                stemChild = true;
                divsChild = divs;
            }else{
                stemChild = false;
        ...
        for (int i = 0; i < this.length; i++) {
            ...
            if(c==null){
                ...
                if(c.stem){
                    ...
                    vis.SetPix(i,RGB(0,0,(c.divs+1)/(MAX_DIVS)));
                }
            }
        }
    }
